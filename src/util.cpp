...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Vdoo
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Vdoo
    // Mac: ~/Library/Application Support/Vdoo
    // Unix: ~/.Vdoo
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Vdoo";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "Vdoo.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty Vdoo.conf if it does not exist
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "Vdood.pid"));
